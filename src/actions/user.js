...
import { generateSubscriptionKey, generateEncryptedCiphertext } from '../util/crypto';
import { RELAY_REQUEST_CONNECT, RELAY_ERROR, RELAY_SEND_CHAT_MESSAGE, RELAY_MARK_READ_CHAT_MESSAGE } from './webrelay';
import { getRecipientPublicKeyB64 } from '../util/request';
...
export const markChatMessageRead = (chatMessage => {
  return async function(dispatch, getState) {
    const messageId = chatMessage.id;

    dispatch({
      type: RELAY_MARK_READ_CHAT_MESSAGE,
      messageId
    })
  }
})

export const connectToWebRelay = (peerId) => {
  ...
  return async function(dispatch, getState) {
    ...
    try {
      subscriptionKey = await generateSubscriptionKey(peerId);

      dispatch({
        type: RELAY_REQUEST_CONNECT,
        userId,
        subscriptionKey,
      });
    } catch (e) {
...
export const sendChatMessage = (recipientPeerId, message) => {
  return async function(dispatch, getState) {
    let encryptedMessage;

    let recipientPublicKeyB64 = await getRecipientPublicKeyB64(recipientPeerId);
    const date = new Date();
    const messageId = uuidv4();

    try {
      encryptedMessage = await generateEncryptedCiphertext(recipientPublicKeyB64, message, date);

      dispatch({
        type: RELAY_SEND_CHAT_MESSAGE,
      ...
      console.error(e)
      dispatch({type: RELAY_ERROR, error: e});
    }
  }
}
